...
function ProductList() {
    ...
    const { paginatedProducts, isLoaded } = useProduct();

    return (
        <Container className="text-center">
            <Row xs={1} md={2} lg={3} xl={4} className="g-2">
                {isLoaded ? paginatedProducts.length === 0 ? <Col>Please select a category to see products</Col> : paginatedProducts.map((product) =>
